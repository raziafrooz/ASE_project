...
recount3_metadata2<-as_tibble(recount3_metadata) %>% 
  filter(experiment_acc %in% final_no_annot$experiment_acc) %>% 
...
for(n in c(1:length(sra_metadata))){
  ...
  if(sum(is.na(mtD)) == length(mtD) | mtD == "" ){
    mtD_df_list[[n]] <- NA
  } else{
...
for(i in c(1:length(tissue_if_found))){
  if(length(is.na(tissue_if_found[[i]]))>=1){
...
# diseases_if_found[["SRP040472"]]<-NA

i <- 1
study <- sra[[i]]$study
run <- sra[[i]]$external_id
experiment_acc <- sra[[i]]$experiment_acc
...
tissue_df <- data.frame(cbind(study,run,experiment_acc, tissue, organ, biopsy, cell, disease, des))
for(i in 2:length(sra)){
  ...
  tmp <- data.frame(cbind(study,run,experiment_acc, tissue, organ, biopsy, cell, disease, des))
  tissue_df <- rbind(tissue_df, tmp)
}

write.csv(tissue_df, "not_annotated.csv")
