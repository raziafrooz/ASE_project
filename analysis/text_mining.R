...
no_annotations<-final_ase[is.na(final_ase$sample_type),]

recount3_metadata2<-recount3_metadata %>% select(study,experiment_acc,external_id,study_abstract,study_title,library_selection,sample_attributes,sample_description)
sra<-split(recount3_metadata2,1:nrow(recount3_metadata2))
names(sra)<-recount3_metadata$experiment_acc
...
for(i in c(1:length(diseases_if_found))){
  if(length(is.na(diseases_if_found[[i]]))>=1){
...
for(i in c(1:length(tissue_if_found))){
  if(sum(is.na(tissue_if_found[[i]]))>=1){
...
write.csv(tissue_df, "/dcs07/hansen/data/recount_ASE/data/sra_text_mining_annotation.csv")
