...
#Penn cohort2
id<-data.frame(indv1=rep(NA,10),indv3=rep(NA,10))
id$indv1[1:6]<-which(sra_geno$sample_id %in% c("SRR8701271","SRR8701272","SRR8701273",
                                    "SRR8701274","SRR8701275","SRR8701276"))

id$indv2<-which(sra_geno$sample_id %in% c("SRR8701277","SRR8701278",
...
#Penn cohort1:
id$indv3[1:8]<-which(sra_geno$sample_id %in% c("SRR8702455","SRR8702456",
...
for(k in id[,i]){
  ...
  ase_all$indv<-indv
  if(indv==colnames(id)[1]){
    ase_all2<-ase_all }else{
...
ase_all2 %>% group_by(indv,sample_id) %>% 
...
#Get GTEx liver
#------------------------------------------------------------
load("~/hansen_lab/ASE/test_ASE/Liver_ase_MS.rda") #names ase_df
ase_df

colnames(ase_df)[6]<-"sample_id"
gtex<-ase_df %>% filter(sample_id %in% unique(sample_id)[1:3])
...
for(i in c(8,16,30,60)){
  if(i==8){
    plot_gtex<-gtex %>% group_by(sample_id) %>%  filter(total>=i) %>% summarise(n())}else{
      rr<-gtex %>% group_by(sample_id) %>%  filter(total>=i) %>% summarise(n())
      plot_gtex<-cbind(plot_gtex,rr[,2])
    }}
colnames(plot_gtex)<-c("sample_id",">=8",">=16",">=30",">=60")
plot_gtex_long<-plot_gtex %>%
...
plot_gtex_long$study<-"GTEX_liver"
plot_geu_long$study<-"Geuvadis"
plot_SRA_long$study<-"SRP187978"

join_plot<-rbind(plot_gtex_long,plot_geu_long,plot_SRA_long)
...
ggplot()+
  geom_jitter(data=join_plot, aes(x=group, y=total,color=study),width = 0.25)+
  scale_x_discrete(limits = level_order)+
  labs(title="Comparison of total number of het_SNPs in 3 samples from Geuvadis vs SRP187978",
       subtitle="Runs for 3 individuals in SRA are combined",
       x="Reads/hetSNP",
...
ase_gr<-makeGRangesFromDataFrame(gtex, seqnames.field="chr",
                                 start.field="pos", end.field="pos")

ov<-findOverlaps(gene_grch38,ase_gr)
gtex$gene_id<-NA
gtex$gene_id[subjectHits(ov)]<-names(gene_grch38)[queryHits(ov)]
...
for(i in c(8,16,30,60)){
  if(i==8){
    plot_gtex<-gtex %>% group_by(sample_id) %>%  filter(total>=i) %>% summarise(length(unique(gene_id)))}else{
      rr<-gtex %>% group_by(sample_id) %>%  filter(total>=i) %>% summarise(length(unique(gene_id)))
      plot_gtex<-cbind(plot_gtex,rr[,2])
    }}
colnames(plot_gtex)<-c("sample_id",">=8",">=16",">=30",">=60")
plot_gtex_long<-plot_gtex %>%
