...
for(k in 1:length(id)){
  ...
  study<-sra_geno$study[id][k]
  geno_dir<-sra_geno$genotypedSamples[id][k]
...
#plot
#-------------------
sra_geno<-sra_geno[which(sra_geno$study == "SRP187978"),]
for(k in 374:nrow(sra_geno)){
  print(k)
  ...
  if(k==1){
    ase_all_liver<-ase_df
  }
  ase_all_liver<-rbind(ase_all_liver,ase_df)
  }}

recount3_metadata[1,]
ase_all_liver$seq_type<-recount3_metadata$seq_type[match(ase_all_liver$sample_id,recount3_metadata$external_id)]
ase_all_liver<-ase_all_liver[which(ase_all_liver$seq_type=="bulk"),]
ase_all_liver$pred.type<-SRA_metadata$pred.type[match(ase_all_liver$sample_id, SRA_metadata$external_id)]
ase_all_liver$study<-SRA_metadata$study[match(ase_all_liver$sample_id, SRA_metadata$external_id)]

table(xx$pred.type)
table(recount3_metadata$seq_type[match(ase_all_liver$sample_id, recount3_metadata$external_id)])
which(SRA_metadata$external_id=="DRR128998")
pdf(file="~/plot/ASE/SRA_normalLiver_pred_seq2.pdf", width = 10, height = 4)
for (i in 2:length(unique(ase_all_liver$study))) {
  study_num<-"SRP187978"#unique(ase_all_liver$study)[i]
  print(study_num)
  xx<- ase_all_liver%>% filter(study==study_num)# %>% group_by(sample_id,pred.type ) %>% summarize(is.median=median(ref_ratio)) %>% ungroup()
  p0=ggplot(xx, aes(x=sample_id,y=ref_ratio))+
    ...
    labs(title=paste0("SRA normal liver samples,", "Study: ", study_num),
         subtitle= " colored based on recount3 predicted seq type")+
...
pdf(file="~/plot/ASE/mono_allelic_filter_liver.pdf", width = 10, height = 4)
for(i in unique(ase_all_liver$sample_id)[1:10]){
  print(i)
  x<-ase_all_liver %>% filter(sample_id==i)%>% rowwise() %>% mutate(mine_allele=min(alt,ref))
  p=ggplot(x)+
    geom_point(aes(x=total, y=mine_allele))+xlim(0,100)+ylim(0,100)+
    labs(title="First 10 liver samples in SRP187978 study: mono-allele check")
  print(p)
}
dev.off()

sum_sam<-ase_all_liver %>%  group_by(sample_id) %>% summarize(min=min(total), median= median(total), max=max(total))
  
sum_sam

pdf(file="~/plot/ASE/liver_coverage.pdf", width = 10, height = 4)
for(i in unique(ase_all_liver$sample_id)[1:10]){
  x<-ase_all_liver %>% filter(sample_id==i)
  p=ggplot(x)+
    geom_histogram(aes(total))+
    xlim(c(0,1000))+
    labs(title="Liver SRP187978 study coverage distribution",
