...
remove_problematic_SNPs<-function(ase_df){
  ...
  id_rm<-which(ase_df$MAPPING_BIAS_SIM>0 | ase_df$MAPPING_BIAS_SIM>0)
  n2<-length(id_rm)
  if(length(id_rm)>0){
  ase_df<-ase_df[-id_rm,]
  }
...
get_error_p<-function(ref_count,coverage,weight1=0.98,weight2=0.02,median_ratio=0.86){
